...
function my_pagination($total, $limit, $page_limit, $page, $param){
    $total_page = ceil($total / $limit); // 소수점이 있다면 무조건 올리는함수

    $start_page = ((floor(($page - 1) / $page_limit)) * $page_limit) + 1; // 소수점이 있다면 무조건 내리는함수
    ...
    $rs_str .= '<li class="page-item"><a class="page-link" href="'.$_SERVER['PHP_SELF'].'?page=1'.$param.'">First</a></li>'; // $_SERVER['PHP_SELF'] = /xampp/member/admin/member.php
